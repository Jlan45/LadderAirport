...
详见 [deploy/README-agent.md](deploy/README-agent.md)：

```bash
make agent
sudo LADDER_TOKEN='你的密钥' ./scripts/install-agent.sh
# 或
sudo LADDER_TOKEN='secret' ./scripts/install-agent.sh ./bin/ladder-agent
```
